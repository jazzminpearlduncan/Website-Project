...
For this analysis, I will be investigating various different relationships between the titles, genres, and popularity of different songs on Spotify (see section below for details on the data set). I hope to be able to draw conclusions about general artist popularity as well as the popularity of specific expressions in music, potentially reflecting preferences in audiences as well as deliberate decisions across different genres.
...
```{r}
# Loading in libraries for processing string data and data verbs, and making plots

library(dplyr)
library(stringr)
library(ggplot2)

# Using str_detect and 1 reg. exp. to filter for specific genres beginning with the letter R
...
# Create table for the most played artists in the last dataset
...
# Finds the word boy or girl and creates columns for either; each mention is taken with it's genre and counted under the genre
...
# Rearranging data to be manageable with desired graph 
...
This plot displays the difference between mentions of the words 'girl' and 'boy' in their titles by genre, with the counts for either gender and colors to indicate gender as well. I chose to do this plot because I was curious about how and where either sex was mentioned; however, without a context variable (sounds very messy), it doesn't provide much insight into the genres. 
...
## Regular Expressions and str functions
